package br.com.edwin.lima.security.jwt;
...
public class JwtTokenProvider {
    ...
    public TokenVO createAccessToken(String login, List<String> roles) {
        Date now = new Date();
        Date validity = new Date(now.getTime() + validityInMilliseconds);
        var accessToken = getAccessToken(login, roles, now, validity);
        var refreshToken = getRefreshToken(login, roles, now);

        return new TokenVO(login, true, now, validity, accessToken, refreshToken);
    }
